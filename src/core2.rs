...
use crate::{option::{LocomotiveOption, Position}, core::{CurrentElements, Listeners}, native::NativeScroll, Scroll, smooth::SmoothScroll, instance::Instance, els::{Els, MappedEl, ScrollToOption, ScrollToTarget}, element_type::ElementType};
...
#[derive(Clone, Debug)]
pub struct Core {
    namespace: Rc<RefCell<String>>,
    html: Rc<RefCell<Element>>,
    window_height: f64,
    window_width: f64,
    window_middle: Position,
    els: Rc<RefCell<Els>>,
    current_elements: Rc<RefCell<CurrentElements>>,
    listeners: Listeners,
    has_scroll_ticking: bool,
    has_call_event_set: bool,
    check_scroll: Rc<RefCell<Option<Closure<dyn FnMut()>>>>,
    check_resize: Rc<RefCell<Option<Closure<dyn FnMut()>>>>,
    check_event: Rc<RefCell<Option<Closure<dyn FnMut(Event)>>>>,
    set_scroll_to: Rc<RefCell<Option<Closure<dyn FnMut(Event)>>>>,
    instance: Rc<RefCell<Instance>>,
    context: String,
    direction_axis: Rc<RefCell<char>>,
    resize_tick: Rc<RefCell<bool>>,
    scroll_to_els: Option<NodeList>,
...
impl Core {
    pub fn new(mut options: LocomotiveOption) -> Self {
        ...
        let scroll = Core::create_scroll(options.clone(), &window, check_scroll_cb, html.clone(), instance.clone(), direction_axis.clone());
...
impl Core {
    ...
    fn create_scroll(options: LocomotiveOption, window: &Window, check_scroll_cb: Closure<dyn Fn()>, html: Rc<RefCell<Element>>, instance: Rc<RefCell<Instance>>, direction_axis: Rc<RefCell<char>>) -> Option<Scroll> {
    //Setup the `Scroll` Field
        ...
        if (options.smooth && !options.is_mobile)  || 
            ...
            {
                let scroll = SmoothScroll::new(options, html, instance, direction_axis);
                Some(Scroll::Smooth(scroll))
        } else {
                let scroll = NativeScroll::new(options, window, check_scroll_cb);
                Some(Scroll::Native(scroll))
        }
    }
...
impl Core {
    pub fn check_scroll_callback(core: Rc<RefCell<Core>>) {
        ...
        *callback.borrow_mut() = Some(Closure::new(move || {
            let core = core.clone();
            let option = core.as_ref().borrow().scroll.as_ref().unwrap().get_option();
    ...
    fn set_scroll_to_callback(core: Rc<RefCell<Core>>, option: &LocomotiveOption) {
        ...
        *callback.borrow_mut() = Some(Closure::new(move |event: Event| {
            ...
            let scroll = core.as_ref().borrow().scroll.clone().unwrap();
